...
const resolvePostcssConfig = async (config) => {
  ...
  const inlineOptions =
    (_config$css = config.css) === null || _config$css === void 0
      ? void 0
      : _config$css.postcss;
...
function vanillaExtractPlugin({ identifiers } = {}) {
  let config;
  let server;
  let postCssConfig;
  ...
  return {
    ...
    async transform(code, id, ssrParam) {
      ...
      const output = await integration.processVanillaFile({
        ...
        filePath: validId,
        identOption:
          identifiers !== null && identifiers !== void 0
            ? identifiers
        ...
        serializeVirtualCssPath: async ({ fileScope, source }) => {
          ...
          if (postCssConfig) {
            const postCssResult = await (
              await Promise.resolve().then(function () {
                return /*#__PURE__*/ _interopNamespace(require("postcss"));
              })
            )
